...
function addFooterToAllPages(doc: any) {
  ...
  for (let i = 0; i < pageCount; i++) {
    doc.switchToPage(i);
    doc.moveTo(40, 790).lineTo(555, 790).strokeColor('#e2e8f0').lineWidth(0.5).stroke();
    doc.fillColor('#64748b').fontSize(8).font('Helvetica')
       .text('Dunga Technologies • Phone: +91 8121923831 • Email: Sales@dungatechnologies.com • www.dungatechnologies.com', 40, 796, { align: 'left' });
    doc.text(`Page ${i + 1} of ${pageCount}`, 450, 796, { align: 'right', width: 105 });
  }
}
...
function renderAgreementPdf(
  ...
  if (LOGO_PATH) {
    try {
      ...
      doc.fillColor(primaryColor).fontSize(20).font('Helvetica-Bold').text('DUNGA TECHNOLOGIES', 40, y);
    }
  } else {
    doc.fillColor(primaryColor).fontSize(20).font('Helvetica-Bold').text('DUNGA TECHNOLOGIES', 40, y);
  }

  doc.fillColor(primaryColor).fontSize(13).font('Helvetica-Bold').text(documentTitle, 230, y + 4, { align: 'right' });
  doc.fillColor(mutedText).fontSize(8.5).font('Helvetica').text('Version: 1.0', 230, y + 20, { align: 'right' });
  doc.text(`Email: Sales@dungatechnologies.com  |  Phone: +91 8121923831`, 230, y + 32, { align: 'right' });
  doc.text(`Ref ID: ${prefixCode}-${(project.id || '').substring(0, 8).toUpperCase()}  |  Date: ${formatDate(new Date())}`, 230, y + 44, { align: 'right' });
  ...
  const ensureSpace = (neededHeight: number) => {
    if (y + neededHeight > 760) {
      ...
      doc.fillColor(primaryColor).fontSize(9).font('Helvetica-Bold').text('DUNGA TECHNOLOGIES — PROJECT DEVELOPMENT AGREEMENT', 40, y);
      doc.fillColor(mutedText).fontSize(8).font('Helvetica').text(`Ref: ${prefixCode}-${(project.id || '').substring(0, 8).toUpperCase()}`, 400, y, { align: 'right' });
      y += 14;
  ...
  const renderSectionHeader = (title: string) => {
    ensureSpace(30);
    doc.rect(40, y, 515, 18).fill('#0f2b5c');
    doc.fillColor('#ffffff').fontSize(9.5).font('Helvetica-Bold').text(title, 48, y + 4);
    y += 24;
  };
  ...
  y += doc.heightOfString(overviewText, { width: 515 }) + 14;
  ...
  doc.fillColor(mutedText).fontSize(8).font('Helvetica-Bold').text('Customer Name:', 48, y + 8);
  doc.fillColor(darkText).font('Helvetica').text(customerName, 130, y + 8);

  doc.fillColor(mutedText).font('Helvetica-Bold').text('Company Name:', 48, y + 22);
  doc.fillColor(darkText).font('Helvetica').text(companyName, 130, y + 22);

  doc.fillColor(mutedText).font('Helvetica-Bold').text('Phone Number:', 48, y + 36);
  doc.fillColor(darkText).font('Helvetica').text(phone, 130, y + 36);

  doc.fillColor(mutedText).font('Helvetica-Bold').text('Email Address:', 48, y + 50);
  doc.fillColor(darkText).font('Helvetica').text(email, 130, y + 50);

  doc.fillColor(mutedText).font('Helvetica-Bold').text('Project Number:', 310, y + 8);
  doc.fillColor(darkText).font('Helvetica-Bold').text(projectNo, 390, y + 8);

  doc.fillColor(mutedText).font('Helvetica-Bold').text('Project Name:', 310, y + 22);
  doc.fillColor(darkText).font('Helvetica-Bold').text(project.projectName || '—', 390, y + 22);

  doc.fillColor(mutedText).font('Helvetica-Bold').text('Project Address:', 310, y + 36);
  doc.fillColor(darkText).font('Helvetica').text(address, 390, y + 36);
  ...
  doc.fillColor(mutedText).fontSize(8).font('Helvetica-Bold').text('Project Name:', 48, y + 8);
  doc.fillColor(darkText).font('Helvetica-Bold').text(project.projectName || '—', 140, y + 8);

  doc.fillColor(mutedText).font('Helvetica-Bold').text('Project Category:', 48, y + 22);
  doc.fillColor(darkText).font('Helvetica').text(project.serviceType || project.projectType || 'Software Development', 140, y + 22);

  doc.fillColor(mutedText).font('Helvetica-Bold').text('Estimated Delivery Time:', 48, y + 36);
  doc.fillColor(darkText).font('Helvetica').text(deliveryTimeStr, 140, y + 36);

  doc.fillColor(mutedText).font('Helvetica-Bold').text('Support Period:', 48, y + 50);
  doc.fillColor(darkText).font('Helvetica').text(project.supportPeriod || 'Standard Technical Support Included', 140, y + 50);

  doc.fillColor(mutedText).font('Helvetica-Bold').text('Project Cost:', 310, y + 8);
  doc.fillColor(darkText).font('Helvetica-Bold').text(formatINR(totalBudget), 400, y + 8);

  doc.fillColor(mutedText).font('Helvetica-Bold').text('Advance Payment:', 310, y + 22);
  doc.fillColor('#059669').font('Helvetica-Bold').text(formatINR(advancePayment), 400, y + 22);

  doc.fillColor(mutedText).font('Helvetica-Bold').text('Balance Amount:', 310, y + 36);
  doc.fillColor('#d97706').font('Helvetica-Bold').text(formatINR(balanceAmount), 400, y + 36);
  ...
  if (project.description) {
    ensureSpace(40);
    doc.fillColor(secondaryColor).fontSize(8.5).font('Helvetica-Bold').text('Project Description:', 40, y);
    y += 12;
    doc.fillColor(darkText).fontSize(8).font('Helvetica').text(project.description, 40, y, { width: 515 });
    y += doc.heightOfString(project.description, { width: 515 }) + 10;
  }
  ...
  if (webList.length || appList.length || adminList.length || timelines.length) {
    ensureSpace(30);
    doc.fillColor(secondaryColor).fontSize(8.5).font('Helvetica-Bold').text('Scope of Work & Key Deliverables:', 40, y);
    y += 12;

    const renderList = (catTitle: string, items: string[]) => {
      if (!items.length) return;
      ensureSpace(items.length * 12 + 15);
      doc.fillColor(darkText).fontSize(8).font('Helvetica-Bold').text(catTitle, 45, y);
      y += 11;
      items.forEach((it: string) => {
        doc.fillColor(darkText).fontSize(8).font('Helvetica').text(`• ${it}`, 55, y, { width: 495 });
        y += 11;
      });
      y += 4;
    };
    ...
    if (timelines.length > 0) {
      ensureSpace(timelines.length * 14 + 20);
      doc.fillColor(darkText).fontSize(8).font('Helvetica-Bold').text('Milestone Deliverables Schedule:', 45, y);
      y += 11;
      timelines.forEach((t: any) => {
        doc.fillColor(darkText).fontSize(8).font('Helvetica').text(`• ${t.description || 'Milestone Phase'} (${t.workingDays || 0} Working Days)`, 55, y, { width: 495 });
        y += 12;
      });
      y += 6;
    }
  ...
  for (let i = 0; i < services.length; i += 2) {
    ensureSpace(12);
    doc.fillColor(darkText).fontSize(8).font('Helvetica').text(services[i], 48, y);
    if (services[i + 1]) {
      doc.text(services[i + 1], 300, y);
    }
    y += 13;
  }
  y += 10;
  ...
  const renderLegalSection = (secNum: string, title: string, points: string[]) => {
    ...
    points.forEach((p, idx) => {
      ...
      y += doc.heightOfString(textLine, { width: 505 }) + 3;
    });
    y += 6;
  };
  ...
  doc.fillColor(darkText).fontSize(8).font('Helvetica')
     .text('By signing below, the Client confirms acceptance of all terms and conditions contained in this Agreement.', 40, y);
  y += 20;

  const sigY = y;
  // Client Signature Box
  doc.rect(40, sigY, 240, 95).strokeColor('#cbd5e1').stroke();
  doc.fillColor(primaryColor).fontSize(8.5).font('Helvetica-Bold').text('CLIENT SIGNATURE', 48, sigY + 8);
  doc.fillColor(darkText).fontSize(8).font('Helvetica').text(`Name: ${customerName}`, 48, sigY + 26);
  doc.text(`Date: ________________________`, 48, sigY + 44);
  doc.text(`Signature: ____________________`, 48, sigY + 66);

  // Dunga Technologies Signature Box
  doc.rect(315, sigY, 240, 95).strokeColor('#cbd5e1').stroke();
  doc.fillColor(primaryColor).fontSize(8.5).font('Helvetica-Bold').text('AUTHORIZED SIGNATORY', 323, sigY + 8);
  doc.fillColor(secondaryColor).fontSize(8).font('Helvetica-Bold').text('DUNGA TECHNOLOGIES', 323, sigY + 20);
  doc.fillColor(darkText).fontSize(8).font('Helvetica').text(`Name: ________________________`, 323, sigY + 36);
  doc.text(`Designation: __________________`, 323, sigY + 52);
  doc.text(`Date: ________________________`, 323, sigY + 68);
  doc.fillColor(mutedText).fontSize(7.5).font('Helvetica-Oblique').text('(Company Seal)', 465, sigY + 80);
...
export async function buildPaymentReceiptPdfBuffer(receiptData: {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.fillColor(mutedText).fontSize(8.5).font('Helvetica').text(`Receipt Date: ${formatDate(receiptData.date || new Date())}`, 250, topY + 26, { align: 'right' });
