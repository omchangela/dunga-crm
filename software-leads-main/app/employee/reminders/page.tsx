...
export default function EmployeeRemindersPage() {
  ...
  async function load() {
    try {
      const data = await employeePortalApi.listReminders();
      const all: any[] = Array.isArray(data?.reminders)      ? data.reminders
        : Array.isArray(data?.data?.reminders)               ? data.data.reminders
  ...
  async function handleToggleFollowUp(leadId: string) {
    try {
      await employeePortalApi.toggleFollowUp(leadId);
      await load();
  ...
  async function handleSaveReminder(leadId: string, iso: string, note: string) {
    try {
      await employeePortalApi.createReminder(leadId, { reminderAt: iso, note: note || "Follow up" });
  ...
  async function handleSaveReReminder(id: string, iso: string, note: string) {
    ...
    try {
      await employeePortalApi.createReReminder(id, { reminderAt: iso, note });
  ...
  async function handleMarkDone(id: string) {
    try {
      await employeePortalApi.markReminderDone(id);
      await load();
  ...
  async function handleDelete(id: string) {
    try {
      await employeePortalApi.deleteReminder(id);
      await load();
    ...
    <>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl">
            ...
            <div className="space-y-3 p-5">
              ...
              {viewLead && (
                <div className="pt-1">
                  <Link href={`/employee/leads/${viewLead.id}`} onClick={() => setViewId(null)}
      ...
      <div className="space-y-5">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <PageHeader title="Reminders" subtitle="Follow-up reminders across your leads." />
                      ...
                      return (
                        <tr key={r.id}
                          ...
                          <td className="px-4 py-3">
                            {lead ? (
                              <Link href={`/employee/leads/${lead.id}`} onClick={(e) => e.stopPropagation()}
                          ...
                          <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                            <div className="space-y-1">
                              <div className="flex flex-wrap items-center gap-1.5">
                                <p className={`whitespace-nowrap text-sm ${overdue ? "font-semibold text-red-600" : "text-[#1a2035]"}`}>
                                  {formatDateTime(r.reminderAt)}
                                </p>
                                {overdue && (
                                  <span className="rounded-full bg-red-100 px-1.5 py-0.5 text-[10px] font-medium text-red-600">Overdue</span>
                                )}
                            ...
                            <div className="flex items-center justify-end gap-2">
                              ...
                              {!isDone && (
                                <button
                                  ...
                                  <CalendarDays className="h-3.5 w-3.5" />Re-Reminder
                                </button>
                              )}
                              {!isDone && (
                                <button onClick={() => handleMarkDone(r.id)}
                                  ...
                                  <Circle className="h-3 w-3" />Completed
                                </button>
                              )}
