...
import {
  ...
  Users, Pencil, ChevronDown, FileSpreadsheet,
} from "lucide-react";
...
function exportCSV(customers: any[]) {
  ...
  a.href = url; a.download = `my_customers_${new Date().toISOString().slice(0, 10)}.csv`;
...
function exportXLSX(customers: any[]) {
  ...
  XLSX.utils.book_append_sheet(wb, ws, "My Customers");
  XLSX.writeFile(wb, `my_customers_${new Date().toISOString().slice(0, 10)}.xlsx`);
}
...
export default function EmployeeCustomersPage() {
  ...
  async function load() {
    ...
    try {
      const data = await employeePortalApi.customers();
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[#1a2035]">My Customers</h1>
          ...
          {showExportMenu && (
            <>
              ...
              <div className="absolute right-0 z-20 mt-1 w-44 overflow-hidden rounded-xl border border-[#e5e9f2] bg-white py-1 shadow-lg">
                ...
                <button onClick={() => { exportXLSX(filtered); setShowExportMenu(false); }}
                  className="flex w-full items-center gap-2.5 px-4 py-2 text-sm text-gray-700 hover:bg-[#f5f6fa]">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-[#f5f6fa]">
                    {["#", "Name", "Total Projects", "Project Pipeline", ""].map((h, i) => (
                      <th key={i} className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-[#8094ae]">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                      ...
                      return (
                        <tr key={c.id}
                            ...
                            {(() => {
                              ...
                              return (
                                <div className="flex items-center gap-1.5">
                                  {s.pending > 0 && (
                                    <span className="inline-flex items-center gap-1 rounded-full border border-yellow-200 bg-yellow-50 px-2 py-0.5 text-[11px] font-semibold text-yellow-700">
                          ...
                          <td className="relative px-3 py-3">
                            <button onClick={(e) => { e.stopPropagation(); setOpenActionId(isOpen ? null : c.id); }}
                            ...
                            {isOpen && (
                              <>
                                ...
                                <div className="absolute right-10 top-1/2 z-20 w-36 -translate-y-1/2 overflow-hidden rounded-xl border border-[#e5e9f2] bg-white py-1 shadow-lg">
                                  <Link href={`/employee/customers/${c.id}`} onClick={() => setOpenActionId(null)}
                                    className="flex w-full items-center gap-2.5 px-4 py-2 text-sm text-gray-700 hover:bg-[#f5f6fa]">
                                    <Eye className="h-3.5 w-3.5 text-[#8094ae]" />View
                                  </Link>
                                  <Link href={`/employee/customers/${c.id}/edit`} onClick={() => setOpenActionId(null)}
                                    className="flex w-full items-center gap-2.5 px-4 py-2 text-sm text-gray-700 hover:bg-[#f5f6fa]">
              ...
              <div className="flex items-center gap-1">
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
                  <button key={p} onClick={() => setCurrentPage(p)}
                    className={`h-8 min-w-[32px] rounded-lg border px-2.5 text-sm font-medium ${
                      p === currentPage ? "border-[#0971fe] bg-[#0971fe] text-white" : "border-[#e5e9f2] text-gray-600 hover:bg-[#f5f6fa]"
                    }`}>{p}</button>
                ))}
