...
import {
  ...
  Calculator, X, Plus, FileDown, Eye, Loader2,
} from "lucide-react";
...
const AVATAR_COLORS = ["bg-blue-500","bg-purple-500","bg-emerald-500","bg-orange-500","bg-pink-500","bg-teal-500","bg-indigo-500","bg-rose-500"];
...
export default function EmployeeEstimationPage() {
  ...
  useEffect(() => () => { Object.values(pdfIntervalsRef.current).forEach(clearInterval); }, []);

  // Pull the estimation list. Prefer the dedicated endpoint; if it isn't available
  // yet, fall back to the projects list filtered to Pending/Rejected so the
  // add-project → estimation → convert flow still works.
  async function load() {
    setIsLoading(true);
    try {
      const data = await employeePortalApi.estimation();
      const list = Array.isArray(data?.projects) ? data.projects : Array.isArray(data) ? data : null;
      if (list && list.length >= 0) { setProjects(list); return; }
      throw new Error("empty");
    } catch {
      try {
        const pData = await employeePortalApi.projects();
        const all = Array.isArray(pData?.projects) ? pData.projects : Array.isArray(pData) ? pData : [];
        setProjects(all.filter((p: any) => p.status === "Pending" || p.status === "Rejected"));
      } catch { setProjects([]); }
  ...
  async function handleGenerateRowPdf(proj: any, ev: React.MouseEvent) {
    ev.stopPropagation();
    const existing = pdfJobs[proj.id];
    if (existing?.state === "queued" || existing?.state === "waiting" || existing?.state === "active") return;
    try {
      const res = await employeePortalApi.generateEstimationPdf(proj.id);
      ...
      pdfIntervalsRef.current[proj.id] = setInterval(async () => {
        try {
          const status = await employeePortalApi.getEstimationPdfStatus(proj.id, jobId);
          const dd = status?.data ?? status;
          setPdfJobs((p) => ({ ...p, [proj.id]: { jobId, state: dd.state, progress: dd.progress ?? 0, message: dd.message ?? "", result: dd.result ?? null } }));
          if (dd.state === "completed") {
            clearInterval(pdfIntervalsRef.current[proj.id]);
            delete pdfIntervalsRef.current[proj.id];
            showToast("PDF generated");
            await load();
            const signedUrl = dd.result?.signedUrl ?? dd.result?.downloadUrl;
          ...
          if (dd.state === "failed") {
            clearInterval(pdfIntervalsRef.current[proj.id]);
            delete pdfIntervalsRef.current[proj.id];
            showToast(dd.error || "PDF generation failed");
            setPdfJobs((p) => ({ ...p, [proj.id]: { jobId, state: "failed", progress: 0, message: dd.error || "Failed", result: null } }));
          }
  ...
  async function handleViewRowPdf(proj: any, ev: React.MouseEvent) {
    ev.stopPropagation();
    try {
      const res = await employeePortalApi.getEstimationPdf(proj.id);
      ...
      showToast(err?.status === 404 ? "Generate PDF first" : (err?.message ?? "Failed to open PDF."));
    }
  }
  ...
  function handleStatusChange(proj: any, newStatus: string) {
    ...
    employeePortalApi.updateProjectStatus(proj.id, { status: toEnum.status(newStatus) })
      .then(async (res) => {
  ...
  async function handleConfirmConvert() {
    ...
    setConverting(true);
    try {
      const res = await employeePortalApi.updateProjectStatus(selected.id, { status: "CONVERTED", payments: cleaned });
      if (!res.success) throw new Error(res.message);
    ...
    <div className="space-y-5">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-lg overflow-hidden rounded-2xl bg-white shadow-2xl">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {(["Pending", "Converted", "Rejected"] as const).map((s) => (
                    <button key={s} type="button" disabled={selected.status === s}
            ...
            <div className="border-t border-gray-100 px-5 py-3 flex justify-end">
              <button onClick={() => router.push(`/employee/customers/${selected.customerId}`)}
            ...
            <div className="p-5 space-y-4">
              <div className="flex items-center justify-between">
                ...
                <button type="button" onClick={addFinalPayment}
                  className="flex h-6 w-6 items-center justify-center rounded-full bg-purple-600 hover:bg-purple-700 text-white transition-colors shadow-sm" title="Add payment row">
      ...
      <div>
        ...
        <p className="mt-0.5 text-sm text-[#8094ae]">
          {projects.length} pending &amp; rejected project{projects.length !== 1 ? "s" : ""} from your customers
        </p>
      </div>
      ...
      <div className="overflow-hidden rounded-xl border border-[#e5e9f2] bg-white shadow-sm">
        ...
        {isLoading ? (
          ...
          <div className="py-16 text-center">
            ...
            <p className="text-sm text-[#8094ae]">Your customers' Pending or Rejected projects appear here</p>
          </div>
        ) : (
          <>
                          ...
                          <td className="px-4 py-3">
                            {(() => {
                              ...
                              if (busy) return (
                                <div className="flex min-w-[130px] flex-col gap-1">
                                  ...
                                  <div className="h-1 w-full overflow-hidden rounded-full bg-gray-200">
                                    <div className="h-1 rounded-full bg-[#0971fe] transition-all duration-300" style={{ width: `${Math.max(job!.progress || 0, 5)}%` }} />
                                  </div>
                                </div>
                              );
                              ...
                              if (p.estimationPdfUrl) return (
                                <button onClick={(e) => handleViewRowPdf(p, e)} title="View estimation PDF"
                                  className="flex h-8 w-8 items-center justify-center rounded-lg border border-[#e5e9f2] text-[#0971fe] hover:bg-[#f0f6ff]">
                                  <Eye className="h-4 w-4" />
                                </button>
                              );
                              return (
                                <button onClick={(e) => handleGenerateRowPdf(p, e)} title="Generate estimation PDF"
            ...
            <div className="flex flex-col gap-3 border-t border-[#e5e9f2] px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-center gap-3 text-sm text-[#8094ae]">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <select value={rowsPerPage} onChange={(e) => { setRowsPerPage(Number(e.target.value) as 10 | 25 | 50); setCurrentPage(1); }}
